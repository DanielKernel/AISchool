...
- 1）AI基础算法：K-Means (簇心迭代)、KNN (欧氏距离+投票)、PageRank (幂迭代)；
- 2）决策推理算法：决策树核心 (信息增益)、二分图判断 (BFS染色)、最小生成树 (Kruskal等算法)、梯度下降 (线性回归版)、SMOTE (过采样)、模型剪枝（删除环中的一条边去环）、分布式异常训练算法（分布式计算梯度、上传梯度更新模型参数）；
- 3）语音语义等领域算法：卷积算子(滑动窗口)、池化 (Max Pooling、average-pooling)
